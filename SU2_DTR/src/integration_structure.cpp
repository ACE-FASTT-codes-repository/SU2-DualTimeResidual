...
void CIntegration::Time_Integration(CGeometry *geometry, CSolver **solver_container, CConfig *config, unsigned short iRKStep,
                                    unsigned short RunTime_EqSystem, unsigned long Iteration, unsigned long Surf_Poin) {
  unsigned short MainSolver = config->GetContainerPosition(RunTime_EqSystem);
  ...
  if (KindSolver != FEM_ELASTICITY) {
    ...
    switch (config->GetKind_TimeIntScheme()) {
      ...
      case (NO_T_INTEGRATION): 
         solver_container[MainSolver]->No_Iteration(geometry, solver_container, config, Surf_Poin);   
        break;
    }
