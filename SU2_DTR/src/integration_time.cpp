...
void CMultiGridIntegration::MultiGrid_Iteration(CGeometry ****geometry,
  ...
  MultiGrid_Cycle(geometry, solver_container, numerics_container, config,
                  ...
                  Iteration, iZone, iInst, Surf_Poin);
...
void CMultiGridIntegration::MultiGrid_Cycle(CGeometry ****geometry,
  ...
  for (iPreSmooth = 0; iPreSmooth < config[iZone]->GetMG_PreSmooth(iMesh); iPreSmooth++) {
    ...
    switch (config[iZone]->GetKind_TimeIntScheme()) {
      ...
      case EULER_EXPLICIT: case EULER_IMPLICIT: case NO_T_INTEGRATION: iRKLimit = 1; break; }
    ...
    for (iRKStep = 0; iRKStep < iRKLimit; iRKStep++) {
      ...
      Time_Integration(geometry[iZone][iInst][iMesh], solver_container[iZone][iInst][iMesh], config[iZone], iRKStep, RunTime_EqSystem, Iteration, Surf_Poin);
  ...
  /*If multigrid is zero, not doing this part*/
  if ( (iMesh < config[iZone]->GetnMGLevels() && ((Iteration >= config[iZone]->GetnStartUpIter()) || startup_multigrid)) ) {
    ...
    for (unsigned short imu = 0; imu <= RecursiveParam; imu++) {
      if (iMesh == config[iZone]->GetnMGLevels()-2) MultiGrid_Cycle(geometry, solver_container, numerics_container, config, iMesh+1, 0, RunTime_EqSystem, Iteration, iZone, iInst, Surf_Poin);
      else MultiGrid_Cycle(geometry, solver_container, numerics_container, config, iMesh+1, RecursiveParam, RunTime_EqSystem, Iteration, iZone, iInst, Surf_Poin);
    }
