...
void CSolver::SetResidual_RMS(CGeometry *geometry, CConfig *config) {
  ...
  for (iVar = 0; iVar < nVar; iVar++) {
    ...
    if (GetRes_RMS(iVar) != GetRes_RMS(iVar)) {
      cout << "SU2 has diverged (Nan detected)" << endl;
...
void CSolver::SetResidual_RMS(CGeometry *geometry, CConfig *config, bool kind_resEval, unsigned long Surf_Poin) {
  unsigned short iVar, iMarker;

#ifndef HAVE_MPI
  unsigned long Np_surf;
  for (iVar = 0; iVar < nVar; iVar++) {
    Np_surf = 0;
    if (GetRes_RMS(iVar) != GetRes_RMS(iVar)) {
      cout << "SU2 has diverged (Nan detected)" << endl;
    ...
    if ( kind_resEval ) {
      ...
      //           Np_surf += geometry->nVertex[iMarker];
      // }
      
      std::cout << "Number of elements on wall markers : " << Np_surf << std::endl;
      SetRes_RMS(iVar, max(EPS*EPS, sqrt(GetRes_RMS(iVar)/Surf_Poin)));
  ...
  su2double *sbuf_residual, *rbuf_residual, *sbuf_coord, *rbuf_coord, *Coord;
  unsigned long *sbuf_point, *rbuf_point, Local_nPointDomain, Global_nPointDomain, Local_Np_surf, Global_Np_surf,
                iPoint, iVertex, Local_nPointSolid, Global_nPointSolid, Local_nPointBox, Global_nPointBox;
  unsigned short iDim;
  ...
  for (iVar = 0; iVar < nVar; iVar++) sbuf_residual[iVar] = GetRes_RMS(iVar);
  Local_nPointDomain = geometry->GetnPointDomain();
  Local_nPointBox = Surf_Poin;
  
  SU2_MPI::Allreduce(sbuf_residual, rbuf_residual, nVar, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  SU2_MPI::Allreduce(&Local_nPointDomain, &Global_nPointDomain, 1, MPI_UNSIGNED_LONG, MPI_SUM, MPI_COMM_WORLD);
  SU2_MPI::Allreduce(&Local_nPointBox, &Global_nPointBox, 1, MPI_UNSIGNED_LONG, MPI_SUM, MPI_COMM_WORLD);
  
  // if ( rank == MASTER_NODE && kind_resEval )
  // cout << "Number of vertices on solid wall : " << Surf_Poin << endl;
  Local_Np_surf = 0;
  Local_nPointSolid = 0;

  if ( kind_resEval ){

    for (iPoint = 0; iPoint < Local_nPointDomain; iPoint++){
      if (geometry->node[iPoint]->GetSolidBoundary())
        Local_nPointSolid++;
    }

    SU2_MPI::Allreduce(&Local_nPointSolid, &Global_nPointSolid, 1, MPI_UNSIGNED_LONG, MPI_SUM, MPI_COMM_WORLD);
  ...
  for (iVar = 0; iVar < nVar; iVar++) {
    ...
    if ( kind_resEval ) { 
      if ( rank == MASTER_NODE && iVar == 0 )
        cout << "Number of vertices on solid wall: " << Global_nPointSolid << endl;
      SetRes_RMS(iVar, max(EPS*EPS, sqrt(rbuf_residual[iVar]/Global_nPointSolid)));
    } else if ( std::abs(config->Get_Box_Coords(1) - config->Get_Box_Coords(0) ) > EPS ) { 
      if ( rank == MASTER_NODE && iVar == 0 )
        cout << "Number of vertices in the selected Box : " << Global_nPointBox << endl;
      SetRes_RMS(iVar, max(EPS*EPS, sqrt(rbuf_residual[iVar]/Global_nPointBox)));
    } else {
      SetRes_RMS(iVar, max(EPS*EPS, sqrt(rbuf_residual[iVar]/Global_nPointDomain)));
    } 
  ...
  // if ( kind_resEval ){
  //   if ( rank == MASTER_NODE )
  //     cout << "Number of elements on wall markers : " << Global_Np_surf << endl;
  // }
...
vector<vector<double> > CSolver::SetSolution_GradientMagn_Exact_LS(CGeometry *geometry, CConfig *config, vector<vector<double> > sol_exact) {
  
  unsigned short iDim, jDim, iVar, iNeigh;
  unsigned long iPoint, jPoint;
  su2double *Coord_i, *Coord_j, *Solution_i, *Solution_j, ***Grad_Exact,
  r11, r12, r13, r22, r23, r23_a, r23_b, r33, weight, detR2, z11, z12, z13,
  z22, z23, z33, product;
  bool singular = false;
  
  Grad_Exact = new su2double** [geometry->GetnPointDomain()];
  for (iPoint = 0; iPoint < geometry->GetnPointDomain(); iPoint++){
    Grad_Exact[iPoint] = new su2double* [nVar];
    for ( iVar = 0; iVar < nVar; iVar++ )
      Grad_Exact[iPoint][iVar] = new su2double [nDim];
  }
  ...
  for (iPoint = 0; iPoint < geometry->GetnPointDomain(); iPoint++) {
    ...
    /*--- Get consevative solution ---*/
    for (iVar = 0; iVar < nVar; iVar++)
      Solution_i[iVar] = sol_exact[iPoint][iVar];
    ...
    for (iNeigh = 0; iNeigh < geometry->node[iPoint]->GetnPoint(); iNeigh++) {
      ...
      for (iVar = 0; iVar < nVar; iVar++)
        Solution_i[iVar] = sol_exact[jPoint][iVar];
    ...
    for (iVar = 0; iVar < nVar; iVar++) {
      for (iDim = 0; iDim < nDim; iDim++) {
        ...
        Grad_Exact[iPoint][iVar][iDim] = product; 
        // node[iPoint]->SetGradient(iVar, iDim, product);
      }
    }

    AD::SetPreaccOut(Grad_Exact[iPoint], nVar, nDim);
    AD::EndPreacc();
  }
  ...
  Set_MPI_SolutionExact_Gradient(geometry, config, Grad_Exact);

  std::vector<std::vector<double> > Grad_Magn(geometry->GetnPointDomain(), std::vector<double> (nVar,0.0));

  for (iPoint = 0; iPoint < geometry->GetnPointDomain(); iPoint++){
    
    for (iVar = 0; iVar < nVar; iVar++){
      for ( iDim = 0; iDim < nDim; iDim++ ){
        Grad_Magn[iPoint][iVar] += Grad_Exact[iPoint][iVar][iDim]*Grad_Exact[iPoint][iVar][iDim];
      }
      Grad_Magn[iPoint][iVar] = sqrt(Grad_Magn[iPoint][iVar]);
    }  
  }

  for (iPoint = 0; iPoint < geometry->GetnPointDomain(); iPoint++){
    for ( iVar = 0; iVar < nVar; iVar++ ){
        delete [] Grad_Exact[iPoint][iVar];
      }
      delete [] Grad_Exact[iPoint];
    }
