...
void CSinglezoneDriver::StartResComp() {
  ...
  if (rank == MASTER_NODE){
    cout << endl <<"Residual Computation using the Single-zone Driver" << endl;
  }
...
void CSinglezoneDriver::PreprocessDT(unsigned long TimeIter) {
  ...
  if ((config_container[ZONE_0]->GetKind_Solver() ==  EULER) ||
      (config_container[ZONE_0]->GetKind_Solver() ==  NAVIER_STOKES) ||
      (config_container[ZONE_0]->GetKind_Solver() ==  RANS) ) {
      solver_container[ZONE_0][INST_0][MESH_0][FLOW_SOL]->SetInitialDualTime(geometry_container[ZONE_0][INST_0], solver_container[ZONE_0][INST_0], config_container[ZONE_0], TimeIter);
  }
