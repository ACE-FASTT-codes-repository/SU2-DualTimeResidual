...
void CDriver::Postprocessing() {
  ...
  if (rank == MASTER_NODE)
    cout << endl <<"------------------------- Exit Success (SU2_DTR) ------------------------" << endl << endl;
...
void CDriver::StartResComp(){
  ...
  while ( ExtIter < config_container[ZONE_0]->GetnExtIter() ) {
    ...
    /*--- Perform some external iteration preprocessing. ---*/
    if (rank == MASTER_NODE)
      cout << "Performing time iteration " << ExtIter << " for dual time stepping. " << endl;

    PreprocessExtIterDT(ExtIter);
...
void CDriver::PreprocessExtIterDT(unsigned long ExtIter) {
  ...
  if(!fsi) {
    for (iZone = 0; iZone < nZone; iZone++) {
      if ((config_container[iZone]->GetKind_Solver() ==  EULER) ||
          ...
          (config_container[iZone]->GetKind_Solver() ==  RANS) ) {
        for (iInst = 0; iInst < nInst[iZone]; iInst++)
          solver_container[iZone][iInst][MESH_0][FLOW_SOL]->SetInitialDualTime(geometry_container[iZone][INST_0], solver_container[iZone][iInst], config_container[iZone], ExtIter);
      }
    }
  }
...
void CDriver::Output(unsigned long ExtIter) {
  ...
  /*--- write the solution ---*/
  //Adding this only for SU2DTR used for dual time stepping
  if ( config_container[ZONE_0]->GetWrt_Sol_Freq_DualTime() != 1 ) output_files = false;
  else output_files = true;
