...
void CMultizoneDriver::StartResComp() {
  ...
  if (rank == MASTER_NODE){
    cout << endl <<"Residual computation using the Multizone Driver" << endl;
  }
...
void CMultizoneDriver::PreprocessDT(unsigned long TimeIter) {
  ...
  for (iZone = 0; iZone < nZone; iZone++){
    ...
    if ((config_container[iZone]->GetKind_Solver() ==  EULER) ||
        (config_container[iZone]->GetKind_Solver() ==  NAVIER_STOKES) ||
        (config_container[iZone]->GetKind_Solver() ==  RANS) ) {
        if(!fsi) solver_container[iZone][INST_0][MESH_0][FLOW_SOL]->SetInitialDualTime(geometry_container[iZone][INST_0], solver_container[iZone][INST_0], config_container[iZone], TimeIter);
    }
  ...
  /*--- Run a predictor step (I might not need this, but not totally sure, so I just comment it) ---*/ 
