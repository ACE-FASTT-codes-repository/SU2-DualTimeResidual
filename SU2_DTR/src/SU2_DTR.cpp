...
int main(int argc, char *argv[]) {
  ...
  CConfig *config = NULL;
  config = new CConfig(config_file_name, SU2_DTR);
  ...
  //   driver = new CMultiphysicsZonalDriver(config_file_name, nZone, nDim, periodic, MPICommunicator);
    ...
    // }
    // SU2_MPI::Error("The current executable support only single/multizone driver.", CURRENT_FUNCTION);
  }
  
  delete config;
  config = NULL;

  /*--- Launch the external loop for residual evaluation ---*/
